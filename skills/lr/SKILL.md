...
2. Before the first remote action on a host, prefer:
   - `uv run lrt-codex invoke test_connection --args '{"host_name":"my-server"}'`
3. Then run the requested action:
   - `uv run lrt-codex invoke run_command --args '{"host_name":"my-server","command":"uname -a"}'`
   - `uv run lrt-codex invoke upload_file --args '{"host_name":"my-server","local_path":"./a.txt","remote_path":"/tmp/a.txt"}'`
   - `uv run lrt-codex invoke download_file --args '{"host_name":"my-server","remote_path":"/tmp/a.txt","local_path":"./a.txt"}'`
   - `uv run lrt-codex invoke query_audit_logs --args '{"limit":10}'`
4. Summarize:
   - host
   - action
   - success or failure
...
- `lrt-codex invoke` returns JSON on stdout
