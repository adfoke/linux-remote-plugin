...
```bash
uv run linux-remote --help
uv run linux-remote --h
```

常用命令：

```bash
uv run linux-remote list-hosts
uv run linux-remote test-connection my-server
uv run linux-remote run-command my-server "uname -a"
uv run linux-remote upload-file my-server ./a.txt /tmp/a.txt
uv run linux-remote download-file my-server /tmp/a.txt ./a.txt
uv run linux-remote audit-logs
```

审计日志过滤：

```bash
uv run linux-remote audit-logs --limit 10
uv run linux-remote audit-logs --host-name my-server
uv run linux-remote audit-logs --operation-type run_command
uv run linux-remote audit-logs --start-time 2026-03-03T00:00:00Z --end-time 2026-03-03T23:59:59Z
```
